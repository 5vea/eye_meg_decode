...
In this script we create the test set for the zero-shot evaluation with the training trials.
...
Also we only have 10000 test trials, because we do not want to test all possible combinations, but also dont want to draw
...
# model folder
path = "/home/saskia_fohs/mbb_dl_project/models/subj2/transformer_CL_2_1000_0-0003_50"
subj = 2
...
eye_train = eye_labels[train_idx]
gts_train = gts_labels[train_idx]

# draw random indeces for original and true image
np.random.seed(42)
n_trials = 10000
all_idx = np.arange(0, len(eye_train))
...
trial_dict["eye_data"] = eye_train[idx]
trial_dict["label_true"] = gts_train[idx, 0]
trial_dict["image_true"] = gts_train[idx, 1]
...
false_labels = np.random.choice(labels_train, n_trials, replace=True)
...
while mask.any():
    trial_dict["label_false"][mask] = np.random.choice(labels_train, mask.sum(), replace=True)
    mask = trial_dict["label_false"] == trial_dict["label_true"]
